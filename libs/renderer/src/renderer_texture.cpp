...
plat::uint32* Texture::Lock()
{
    ...
    SPDIAG_ASSERT((uintptr_t)rect.pBits % sizeof(plat::uint32) == 0, "Texture block must be 4 byte aligned (%d)", (uintptr_t)rect.pBits % sizeof(plat::uint32));
...
void Texture::Unlock(plat::uint32*& pLockPointer)
{
    ...
    SPDIAG_ASSERT(pLockPointer == m_pLocked, "Passing an invalid pointer to pLockPointer");
