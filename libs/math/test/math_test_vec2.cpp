...
namespace {
    template <typename T>
    UnitTest::MemoryOutStream& operator<<(
        UnitTest::MemoryOutStream& stream
        , const secondpylon::math::vec2<T>& v) {
        stream << "(" << v.x << "," << v.y << ")";
        return stream;
    }
}
...
SUITE(vec2) {
    ...
    TEST(Interpolate) {
        CHECK_EQUAL(vec2f(0, 0),
            vec2f::Interpolate(vec2f(0, 0), vec2f(1, 1), 0));

        CHECK_EQUAL(vec2f(0.5F, 0.5F),
            vec2f::Interpolate(vec2f(0, 0), vec2f(1, 1), 0.5f));

        CHECK_EQUAL(vec2f(1, 1),
            vec2f::Interpolate(vec2f(0, 0), vec2f(1, 1), 1));
    }
